...
status_t TSBuffer::getTSPacket(sp<DataSource> dataSource, uint8_t **data, off64_t dataSrcOffset) {
    if (mSize - mOffset < kTSPacketSize) {
        ...
        if (size < kTSPacketSize) {
            ALOGW("Completed reading, end of file");
            return ERROR_END_OF_STREAM;
        }
        ssize_t retVal = dataSource->readAt(dataSrcOffset, mData, size);
        if (retVal < size) {
            ALOGE("Cannot read data from data source %d, %lld", size, dataSrcOffset);
...
MPEG2TSSource::MPEG2TSSource(
    ...
    ALOGV("Stream PID %d, program PID %d", mStream->mStreamPID, mStream->mProgramPID);
...
status_t MPEG2TSSource::read(
    ...
    *out = NULL;
    ...
    if (seekAble && options && options->getSeekTo(&seekTimeUs, &seekMode)) {
        ...
        ALOGV("Seek PTS %lld , start searching from offset %lld", seekPTS, seekOffset);

        err = findOffsetForPTS(seekOffset, seekPTS);
        if (err != OK) {
            ALOGE("Cannot seek, unable to find offset %lld",seekTimeUs);
            return err;
        }
        ...
        if (mIsVideo) {
            err = seekToSync();
            if (err != OK) {
                ALOGE("Cannot seek this TS clip %d", err);
                return err;
            }
        }
    }
...
status_t MPEG2TSSource::findStreamDuration(){
     ...
     ALOGV("First PTS found %lld, for stream %d, at %lld", mStream->mFirstPTS, mStream->mStreamPID, mStream->mFirstPTSOffset);
     ...
     while (offset > 0) {
         ...
         if (retVal < 0) {
            ALOGE("Error while reading data from datasource");
            return status_t(retVal);
         }
         if (retVal < (ssize_t)kTSPacketSize) {
             ALOGV("Reached end of stream while searchin for last PTS");
             return ERROR_END_OF_STREAM;
         }
         ...
         if (status == OK) {
             ...
             ALOGV("Last PTS found %lld, for stream %d, at %lld", mStream->mLastPTS, mStream->mStreamPID, offset);
             break;
         }
         offset -= kTSPacketSize;
     }
     if (status != OK) {
         ALOGE("Could not find last PTS %d", status);
         return status;
     }
     mStream->mDurationUs = ((mStream->mLastPTS - mStream->mFirstPTS) * 100 )/9;
     CHECK(mStream->mDurationUs != 0);

     ALOGV("Stream duration %lld", mStream->mDurationUs);
...
status_t MPEG2TSSource::findOffsetForPTS(off64_t& seekOffset, uint64_t seekPTS){
    ...
    ALOGV("In prev PTS %lld, curr PTS %lld, actual pTS %lld", prevPTS, currPTS, seekPTS);
    ALOGV("In offset %lld, prev offset %lld ", fileOffset, prevOffset);
    while(!found) {
        retVal = mDataSource->readAt(fileOffset, packet, kTSPacketSize);
        if (retVal <= 0 || retVal < (ssize_t)kTSPacketSize) {
            ALOGW("Error while reading data from datasource");
        ...
        if (status == OK){
             if ((seekPTS == currPTS) ||
                 ...
                 ALOGV("Seek PTS found %lld, for stream %d at %lld", seekPTS, mStream->mStreamPID, fileOffset);
                 found = true;
                 break;
             }
             if (currPTS < seekPTS){
        ...
        if (searchBack) {
            if ((fileOffset > kTSPacketSize) && (fileOffset - kTSPacketSize > mStream->mFirstPTSOffset)) {
                 ...
                 ALOGW("Reached start of file searching for seek PTS %lld", seekPTS);
            ...
            if (fileOffset + kTSPacketSize <= mStream->mLastPTSOffset) {
                 ...
                 ALOGW("Reached end of file searching for seek PTS %lld", seekPTS);
...
status_t MPEG2TSSource::seekToSync() {
    ...
    while(!isSync && status == OK) {
        if (!mImpl->hasBufferAvailable(&finalResult)) {
         ...
         ALOGV("Found a frame at seek offset, check if it is reference frame");
         status = mImpl->nextBufferIsSync(&isSync);
         if (status == OK && !isSync) {
             sp<ABuffer> accessUnit = NULL;
             ALOGI("dropping access unit");
             status = mImpl->dequeueAccessUnit(&accessUnit);
         }
    }

    if (status != OK) {
        ALOGE("Cannot find sync frame for video");
    }
...
MPEG2TSExtractor::MPEG2TSExtractor(const sp<DataSource> &source)
    ...
    if (mClipSize % kTSPacketSize != 0) {
        ALOGW("Clipsize is not correct %lld", mClipSize);
        mClipSize = 0;
        return;
    }
