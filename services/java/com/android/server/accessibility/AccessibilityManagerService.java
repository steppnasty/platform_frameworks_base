...
public class AccessibilityManagerService extends IAccessibilityManager.Stub {
    ...
    private int mCurrentUserId = UserId.USER_ALL;
    ...
    private void registerBroadcastReceivers() {
        PackageMonitor monitor = new PackageMonitor() {
            ...
            public void onPackageRemoved(String packageName, int uid) {
                synchronized (mLock) {
                    final int userId = UserId.USER_CURRENT;
        ...
        // package changes
        monitor.register(mContext, true);
        ...
        synchronized (mLock) {
            ...
            if (mSecurityPolicy.isCallerInteractingAcrossUsers(userId)) {
                AccessibilityConnectionWrapper wrapper = new AccessibilityConnectionWrapper(
                        windowId, connection, UserId.USER_ALL);
                wrapper.linkToDeath();
    ...
    private void restoreStateFromMementoIfNeeded() {
        synchronized (mLock) {
            if (mTempStateChangeForCurrentUserMemento.mUserId != UserId.USER_NULL) {
                UserState userState = getCurrentUserStateLocked();
    ...
    private Service getQueryBridge() {
        if (mQueryBridge == null) {
            ...
            mQueryBridge = new Service(UserId.USER_NULL, null, info, true);
        }
        return mQueryBridge;
    }
     ...
     * @param userId The id of the user owning the connection. UserId.USER_ALL
     *     if global.
     */
    ...
    private void handleAccessibilityEnabledSettingChangedLocked(UserState userState) {
        userState.mIsAccessibilityEnabled = Settings.Secure.getInt(
    ...
    private void handleTouchExplorationEnabledSettingChangedLocked(UserState userState) {
        userState.mIsTouchExplorationEnabled = Settings.Secure.getInt(
                mContext.getContentResolver(),
                Settings.Secure.TOUCH_EXPLORATION_ENABLED, 0) == 1;
    }

    private void handleDisplayMagnificationEnabledSettingChangedLocked(UserState userState) {
        userState.mIsDisplayMagnificationEnabled = Settings.Secure.getInt(
                mContext.getContentResolver(),
    ...
    final class SecurityPolicy {
        ...
        public int resolveCallingUserIdEnforcingPermissionsLocked(int userId) {
            ...
            final int callingUserId = UserId.getUserId(callingUid);
            if (callingUserId == userId) {
                return userId;
            }
            if (userId == UserId.USER_CURRENT
                    || userId == UserId.USER_CURRENT_OR_SELF) {
                return mCurrentUserId;
            }
            throw new IllegalArgumentException("Calling user can be changed to only "
                    + "UserId.USER_CURRENT or UserId.USER_CURRENT_OR_SELF.");
        }
    ...
    private class TempUserStateChangeMemento {
        public int mUserId = UserId.USER_NULL;
        public boolean mIsAccessibilityEnabled;
        public boolean mIsTouchExplorationEnabled;
        public boolean mIsDisplayMagnificationEnabled;
        public final Set<ComponentName> mEnabledServices = new HashSet<ComponentName>();
