...
public final class DisplayManager {
    ...
    private Display getOrCreateDisplayLocked(int displayId, boolean assumeValid) {
        ...
        if (display == null) {
            Log.w(TAG, "getOrCreateDisplayLocked: displayId " + displayId);
            display = mGlobal.getCompatibleDisplay(displayId,
                    mContext.getCompatibilityInfo(displayId));
            Log.w(TAG, "getOrCreateDisplayLocked1: displayId " + displayId + " " + assumeValid);
            if (display != null) {
                mDisplays.put(displayId, display);
                Log.w(TAG, "getOrCreateDisplayLocked2: displayId " + displayId + " " + assumeValid);
            }
        } else if (!assumeValid && !display.isValid()) {
            display = null;
            Log.w(TAG, "getOrCreateDisplayLocked3: display null");
        }
        return display;
    }
